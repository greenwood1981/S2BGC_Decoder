...
void Argo_Mission::Decode(std::vector<uint8_t> &data) {
  ...
  rise_time_max = ((data[11]<<8) + data[12])/60;
  fall2park_time_max = ((data[13]<<8) + data[14])/60;
  park2prof_time_max = ((data[15]<<8) + data[16])/60;
  drift_time_target = ((data[17]<<8) + data[18])*5/60;
  float_version = data[19];
  ascent_speed_target = data[20]; // cm/s
  num_seeks = (data[21]<<8) + data[22]; // number of seek periods
  surf_time_max = double((data[23]<<8) + data[24]) / 3600; // hours
  seek_time_max = double((data[25]<<8) + data[26]) / 60; // hours
